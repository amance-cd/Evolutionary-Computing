...
int family_competition(Population *parent_population, Population *offspring_population, Population *new_population){
    ...
    for (int i = 0; i < parent_population->size; i+=2){
        ...
        float max_parent_fitness = (family[0]->fitness > family[1]->fitness) ? family[0]->fitness : family[1]->fitness;
        if (family[2]->fitness > max_parent_fitness || family[3]->fitness > max_parent_fitness) {
            improvement_found = 1;
        }
